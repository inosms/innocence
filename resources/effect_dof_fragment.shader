...
uniform float focalDepth = 1;  //focal distance value in meters, but you may use autofocus option below
uniform float focalLength = 20; //focal length in mm
uniform float fstop = 2; //f-stop value
...
float threshold = 0.3; //highlight threshold;
...
float vignette()
{
	float dist = distance(out_tex.xy, vec2(0.5,0.5));
...
void main()
{
	...
	float depth = linearize(texture(bgl_DepthTexture,out_tex.xy).x);

	if (depthblur)
	{
		depth = linearize(bdepth(out_tex.xy));
	}
	...
	vec2 noise = rand(out_tex.xy)*namount*blur;
	...
	{
		col = texture(bgl_RenderedTexture, out_tex.xy).rgb;
	}
	...
	else
	{
		...
		for (int i = 1; i <= rings; i += 1)
		{
			...
			for (int j = 0 ; j < ringsamples ; j += 1)
			{
				...
				col += color(out_tex.xy + vec2(pw*w,ph*h),blur)*mix(1.0,(float(i))/(float(rings)),bias)*p;
