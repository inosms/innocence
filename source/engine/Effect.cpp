...
void Effect::SetManager(EffectManager* n_manager){m_manager = n_manager;}

void EffectManager::AddEffect(std::string n_name,Effect* n_effect)
{
	n_effect->SetName(n_name);
    n_effect->SetManager(this);
	m_effects.push_back(std::unique_ptr<Effect>(n_effect));
}
...
Effect* EffectManager::GetEffect(std::string n_name)
{
	...
	{
		if( i->GetName() == n_name ) return i.get();
	}
	return nullptr;
}
