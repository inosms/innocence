...
SceneNode_Test::SceneNode_Test(unsigned int n_id, int n_width) : SceneNode(n_id), m_width(n_width)
{
	if(!g_meshShader)
	{
...
void SceneNode_Test::VRender( double n_interpolation, glm::mat4x4& n_matrix )
{
	...
	glm::mat4x4 tmp_scale = glm::scale(glm::mat4x4(1.f),glm::vec3(m_width,1.f,1.f));
